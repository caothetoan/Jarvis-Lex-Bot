...
def generate_hotel_price(location, nights, table_type):
    """
    Generates a number within a reasonable range that might be expected for a hotel.
    The price is fixed for a pair of location and TableType.
    """

    table_types = ['vip', 'inside', 'outdoor']
    cost_of_living = 0
    for i in range(len(location)):
        cost_of_living += ord(location.lower()[i]) - 97

    return nights * (100 + cost_of_living + (100 + table_types.index(table_type.lower())))



def isvalid_restaurant(city):
	# get from api services
    valid_cities = ['the coffee house', 'kombo',]
    return city.lower() in valid_cities


def isvalid_table_type(table_type):
    table_types = ['vip', 'inside', 'outdoor']
    return table_type.lower() in table_types
...
def validate_restaurant(slots):
    ...
    nights = safe_int(try_ex(lambda: slots['Quantity']))
    table_type = try_ex(lambda: slots['TableType'])

    if location and not isvalid_restaurant(location):
    ...
    if table_type and not isvalid_table_type(table_type):
        return build_validation_result(False, 'TableType', 'I did not recognize that room type.  Would you like to stay in a vip, king, or deluxe room?')
...
def book_restaurant(intent_request):
    ...
    nights = safe_int(try_ex(lambda: intent_request['currentIntent']['slots']['Quantity']))

    table_type = try_ex(lambda: intent_request['currentIntent']['slots']['TableType'])
    ...
    session_attributes['currentBookingTable'] = reservation
    ...
    if intent_request['invocationSource'] == 'DialogCodeHook':
        ...
        if location and checkin_date and nights and table_type:
            # The price of the hotel has yet to be confirmed.
            price = generate_hotel_price(location, nights, table_type)
            session_attributes['currentBookingTablePrice'] = price
        else:
            try_ex(lambda: session_attributes.pop('currentBookingTablePrice'))
    ...
    try_ex(lambda: session_attributes.pop('currentBookingTablePrice'))
    try_ex(lambda: session_attributes.pop('currentBookingTable'))
    ...
    return close(
        ...
        'Fulfilled',
        {
            ...
            'content': 'Thanks, I have placed your reservation.   Please let me know if you would like to book an other table'
                       '.'
        }
    )
